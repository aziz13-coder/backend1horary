...
def create_production_server(host='127.0.0.1', port=None):
    """Create a production-ready WSGI server"""
    if port is None:
        try:
            port = int(os.getenv('HORARY_PORT', '52525'))
        except Exception:
            port = 52525
...
def run_production_server():
    ...
    try:
        ...
        logger.info(f"Production server starting on http://{server.server_address[0]}:{server.server_address[1]}")
